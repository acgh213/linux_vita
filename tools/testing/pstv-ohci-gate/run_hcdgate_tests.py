#!/usr/bin/env python3
"""Compile the actual HCD-gate production file against host API doubles and run it."""
...
HEADERS = ("capability completion debugfs delay dma-mapping interrupt io ioport "
           "irqdomain module mutex of_irq of_platform platform_device pm_runtime "
           "seq_file slab suspend uaccess usb usb/hcd types")

def main():
    cc = shutil.which("cc") or shutil.which("gcc")
    if not cc:
        print("compiler not found", file=sys.stderr); return 2
    with tempfile.TemporaryDirectory(prefix="pstv-ohci-hcdgate-host-") as td:
